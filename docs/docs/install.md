...
Common options for the Hilbert spacee are {class}`~nk.hilbert.Spin`,  {class}`~nk.hilbert.Fock` or {class}`~nk.hilbert.QuBit`, but it is also possible to define your own.
